class ReallyAnnoyingWayToAddTriggers < ActiveRecord::Migration
  def change
    ...
    sql = <<-SQL
      ...
      IF EXISTS(SELECT * FROM debater_stats WHERE debater_id = NEW.debater_id and year =
        (SELECT year FROM tournaments WHERE tournaments.id = NEW.tournament_id))
        THEN UPDATE debater_stats
        SET soty_points = soty_points + soty_points(NEW.speaker_award),
        noty_points = noty_points + noty_points(NEW.novice_speaker_award)
